...
def primal_optimizer(hyperparam_vect, i_hyper):
    ...
    grad_indexed_loss_fun = grad(indexed_loss_fun)
    W_opt = sgd_parsed(grad_indexed_loss_fun, kylist(W0, alphas, betas, L2_reg), parser, callback=callback)
...
def meta_callback(hyperparam_vect, i_hyper, metagrad=None):
    ...
    meta_results['tests_loss'].append(loss_fun(x, **test_data))
    meta_results['test_err'].append(frac_err(x, **test_data))
...
_ = ax.set_ylabel('Learning rate')
_ = ax.set_xlabel('Schedule index')
fig.set_size_inches((6,2.5))
ax.legend(numpoints=1, loc=1, frameon=False, prop={'size':'12'})
plt.savefig('schedules_small.png', pad_inches=0.05, bbox_inches='tight')
